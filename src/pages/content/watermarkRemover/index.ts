 ...
 * Scopes to `main` or the conversation container when possible to avoid
 * reacting to every DOM mutation in Gemini's SPA shell (sidebar, header, etc.).
 */
const setupMutationObserver = (): void => {
  ...
  const observeTarget =
    ...
    document.body;
  new MutationObserver(debouncedProcess).observe(observeTarget, {
    childList: true,
    subtree: true,
