...
import { TimelineStarManager } from './TimelineStarManager';
...
export class TimelineManager {
  ...
  private starManager: TimelineStarManager | null = null;
  ...
  async init(): Promise<void> {
    ...
    // Initialize star manager with delegate callbacks
    this.starManager = new TimelineStarManager({
      getConversationId: () => this.conversationId,
      getConversationTitle: () => this.getConversationTitle(),
      getMarkerSummary: (turnId: string) => this.markerMap.get(turnId)?.summary,
      onStarredSetChanged: (newSet: Set<string>) => {
        this.starred = newSet;
        this.updateStarredUI();
      },
    });
    await this.starManager.load();
    this.starred = new Set(this.starManager.getStarredIds());
    await this.starManager.syncFromService();
    this.starred = new Set(this.starManager.getStarredIds());
  ...
  /**
   * Update marker UI to reflect current starred state.
   * Called by TimelineStarManager via the delegate callback.
   */
  private updateStarredUI(): void {
    for (const marker of this.markers) {
      const want = this.starred.has(marker.id);
      if (marker.starred !== want) {
        marker.starred = want;
  ...
  private applySharedStarredData(data?: StarredMessagesData | null): void {
    ...
    if (this.starManager) {
      this.starManager.applySharedData(data);
      return;
    }
  ...
  private setupEventListeners(): void {
    ...
    this.onStorage = (e: StorageEvent) => {
      ...
      if (this.starManager) {
        this.starManager.applySet(nextSet, false);
      } else {
    ...
    if (typeof chrome !== 'undefined' && chrome.storage?.onChanged) {
      this.onChromeStorageChanged = (changes, areaName) => {
        ...
        if (this.starManager) {
          this.starManager.applySharedData(starredChange.newValue as StarredMessagesData | null);
        } else {
    ...
    this.eventBusUnsubscribers.push(
      eventBus.on('starred:removed', ({ conversationId, turnId }) => {
        ...
        if (this.starred.has(turnId)) {
          // Build the next set with the turnId removed
          const nextSet = new Set(this.starred);
          nextSet.delete(turnId);
          if (this.starManager) {
            this.starManager.applySet(nextSet);
          } else {
  ...
  private async toggleStar(turnId: string): Promise<void> {
    if (!this.starManager) return;
    await this.starManager.toggle(turnId);
  }
